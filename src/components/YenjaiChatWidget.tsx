...
const YenjaiChatWidget: React.FC<YenjaiChatWidgetProps> = ({ patientCase, contextMessages }) => {
  ...
  return (
    <div id="tour-yenjai" className="fixed bottom-6 right-6 z-[9999] flex flex-col items-end font-body-md">
      ...
      {!isOpen && (
        <>
          {/* iPad & Mobile: Icon-only circular mascot button */}
          <button 
            onClick={() => setIsOpen(true)}
            className="lg:hidden relative group flex items-center justify-center w-14 h-14 md:w-16 md:h-16 rounded-full bg-primary-container border-2 border-primary shadow-xl hover:scale-110 transition-all p-1"
          ...
          {/* Desktop (lg+): Original Full LIVE CHAT Banner Button */}
          <button 
            onClick={() => setIsOpen(true)}
            className="hidden lg:flex relative group items-center transition-all hover:scale-105 animate-bounce-slow"
          >
