...
// Read from cases.ts but parse it roughly or just use the data
const lowCase = {
    id: 'gen_covid_low_1',
    tier: 'Low',
...
async function fix() {
  await setDoc(doc(db, 'cases', 'gen_covid_low_1'), lowCase);
  await setDoc(doc(db, 'cases', 'gen_covid_high_1'), highCase);
  console.log('Fixed Firestore');
  process.exit(0);
}
fix();
